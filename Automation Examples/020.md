...
In order to do this, add "https:" prefix to the "src" and use the `file.download` command.
...
It does so because the webpage isn't fully loaded and some of the elements are not yet present so the script can't continue. To load more elements, you should scroll. Insert a below line at the end of the loop to achieve this.
...
Make sure to enable addon msoffice from the View/Addons panel.
